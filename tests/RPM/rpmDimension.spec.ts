...
test.describe('RPM Dimension', () => {
	...
	test.describe('Create new Dimension', () => {
		...
		test('Find the new dimension in the table', async () => {
			...
			const cols = page.locator('.table.table-compact').locator('tr').locator('td');
			await expect(cols.first()).toHaveText(dimension);
			await expect(cols.nth(1)).toHaveText('Test dimension');
			await expect(cols.nth(2)).toHaveText('L(0,0)M(0,0)T(0,0)I(0,0)Θ(0,0)N(0,0)J(0,1)');
		});
	});
	...
	test.describe('Edit new Dimension', () => {
		...
		test('Find the edited dimension in the table', async () => {
			...
			await expect(cols.first()).toHaveText(dimension);
			await expect(cols.nth(1)).toHaveText('Test dimension edited');
			await expect(cols.nth(2)).toHaveText('L(0,0)M(0,0)T(0,0)I(0,0)Θ(0,0)N(0,0)J(0,1)');
		});
	});
